...
# Named providers (with fallback order). These subcommands write
# ~/.config/mu/ask-config.json; --config is not wired up for them yet (#54).
...
# Removing a provider is refused while a module still points at it, so re-point
# the module first or drop it from the provider list
mu set ask module --provider deepseek
...
defaults; `--config <path>` selects the file that is written.

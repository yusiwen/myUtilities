...
The command is available as `mu network http`.
...
mu network http -H "X-Token: abc" -H "Accept: application/json" https://api.example.com

# Bearer auth
mu network http -A "Bearer token123" https://api.example.com/me

# Force pretty-print for JSON responses
mu network http -j https://api.example.com/data

# Pipe a body from stdin
cat payload.json | mu network http -X POST https://api.example.com/items

# Write the body to a file instead of stdout
mu network http -o response.json https://api.example.com/data

# Body only (no status line or headers)
mu network http -b https://api.example.com/data

# Skip TLS verification (self-signed certs)
mu network http -k https://self-signed.local/api

# Do not follow redirects
mu network http -N https://example.com/redirect
```
